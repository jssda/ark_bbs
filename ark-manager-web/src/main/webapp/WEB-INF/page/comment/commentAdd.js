layui.config({
    ...
    //创建一个编辑器
    var editIndex = layedit.build('comContent', {
        height: 200,
    ...
    layedit.setContent(editIndex, $("#comContent").text(), true);
    //用于同步编辑器内容到textarea
    layedit.sync(editIndex);

    //渲染下拉框插件
    var comUserName = xmSelect.render({
        el: '#comUserName',
        language: 'zn',
        name: 'comUserId',
        tips: '输入搜索用户或者选择',
    ...
    form.on("submit(addComment)", function (data) {
        ...
        // 富文本编辑器中的内容
        data.field.comContent = layedit.getContent(editIndex).split('<audio controls="controls" style="display: none;"></audio>')[0];

        // 提交信息
        $.ajax({
            url: "http://localhost:8080/manager/comment/addComment",
            type: "POST",
