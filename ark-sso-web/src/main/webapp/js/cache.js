...
layui.use(['form', 'jquery', "layer"], function () {
    ...
    var form = layui.form, // 表单模块
        ...
        layer = parent.layer === undefined ? layui.layer : top.layer; // 加载layer模块, 如果父模块已经加载了, 取得父模块的layer组件

    //判断是否web端打开
    if (!/http(s*):\/\//.test(location.href)) {
        layer.alert("请先将项目部署到 localhost 下再进行访问【建议通过tomcat、webstorm、hb等方式运行，不建议通过iis方式运行】，" +
            "否则部分数据将无法显示");
    ...
    function showNotice() {
        layer.open({
            ...
            btn: ['火速围观'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;' +
                'border-bottom:1px solid #e2e2e2;"><p class="layui-red">' +
                '请使用模版前请务必仔细阅读首页右下角的《更新日志》，避免使用中遇到一些简单的问题造成困扰。</p>' +
                '</pclass></p><p>1.0发布以后发现很多朋友将代码上传到各种素材网站，当然这样帮我宣传我谢谢大家，' +
                '但是有部分朋友上传到素材网站后将下载分值设置的相对较高，需要朋友们充钱才能下载。本人发现后通过和站长、' +
                '网站管理员联系以后将分值调整为不需要充值才能下载或者直接免费下载。在此郑重提示各位：' +
                '<span class="layui-red">本模版已进行作品版权证明，不管以何种形式获取的源码，请勿进行出售或者上传' +
                '到任何素材网站，否则将追究相应的责任。</span></p></div>',
