layui.config({
    ...
    //创建一个编辑器
    let editIndex = layedit.build('secTitle', {
        height: 200,
    ...
    // 创建编辑器的时候, 将textArea中的数据同步到编辑器中, 以追加方式添加
    layedit.setContent(editIndex, $("#secTitle").text(), true);
    //用于同步编辑器内容到textarea
    layedit.sync(editIndex);

    //渲染下拉框插件
    var secUserName = xmSelect.render({
        el: '#secUserName',
        language: 'zn',
        name: 'secUserId',
        tips: '输入搜索用户或者选择',
        ...
        data: function () {
            ...
            if ($("#userId").val() != -1 && $("#userName").val() != -1) {
                userList = [];
                userList.push(
                    {userName: $("#userName").val(), userId: $("#userId").val(), selected: true}
                )
            } else {
                if (userList == null) {
                    $.ajax({
    ...
    form.on("submit(addSection)", function (data) {
        ...
        // 富文本编辑器中的内容
        data.field.secTitle = layedit.getContent(editIndex).split('<audio controls="controls" style="display: none;"></audio>')[0];

        // 提交信息
        $.ajax({
            url: "http://localhost:8080/manager/section/addOrEditSection",
            type: "POST",
